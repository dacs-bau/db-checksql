...
buildah config --author "Olaf Mersmann <olafm@p-value.net>" --label "name=db-ws2024-api-server" "$ctr"
...
Run
  ...
  podman push --format v2s2 db-ws2024-api-server:latest docker://registry.fly.io/db-ws2024-api-server:latest

to deploy app to fly.io
EOF
